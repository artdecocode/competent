 ...
 * @param {(componentName: string) => ?} [conf.onSuccess] The callback at the end of a successful replacement with the component's key.
 * @param {(componentName: string, error: !Error, position: number, input: string) => ?} [conf.onFail] The callback at the end of failed replacement with the component's key, error object, position number and the string which was fed to the rule.
 ...
 * @prop {(componentName: string) => ?} [onSuccess] The callback at the end of a successful replacement with the component's key.
 * @prop {(componentName: string, error: !Error, position: number, input: string) => ?} [onFail] The callback at the end of failed replacement with the component's key, error object, position number and the string which was fed to the rule.
 ...
 * @prop {(shouldExport?: boolean) => ?} export When called, marks the component for export and adds an `id` if the root element of the hyper result did not have it. Individual instances can pass the `false` value if they don't want to get exported.
 * @prop {(isPretty: boolean, lineLength?: number) => ?} setPretty The function which controls whether to enable pretty printing, and the line width.
 * @prop {(doRender?: boolean, recursiveRender?: boolean) => ?} renderAgain After rendering the component itself, the children by default are also rendered by spawning another _Replaceable_ stream. This is needed when a component might contain other components when rendered.
